...
/** Classe controle responsavel pela disputa do 3 lugar
* 
* @author Weslei Silva Santos
*
*/
public class CRUD3LugarController {
    ...
    void CriarAction(ActionEvent event) {
    	...
    	ArrayList<Integer> IdsPerdedoras = new ArrayList<Integer>();
    	//Se as partidas da semifinal ja foram criadas e editadas
    	if(semiDao.ListPart().size()==2 && verificar) {
    		// Se a disputa pelo terceiro lugar nao foi criada
    		if(tercLugarDao.ListPart().size()==0) {
	    		int idPerdedora = 0;
				for(Partida partida: semiDao.ListPart()){
		    		// Verificando selecoes ganhadoras das semifinais
					if(partida.getGolsSelecao1()<partida.getGolsSelecao2()){
		    			idPerdedora = partida.getSelecao1();}
		    		if(partida.getGolsSelecao1()>partida.getGolsSelecao2()) {
		    			idPerdedora = partida.getSelecao2();}
		    		// Adicionando o id das selecoes vencedoras em uma lista
		    		IdsPerdedoras.add(idPerdedora);}
					
				//Fazendo as partidas com base no chaveamento da copa
				int idSelecao1 = IdsPerdedoras.get(0); 
		    	int idSelecao2 = IdsPerdedoras.get(1);
	    		Partida partida = new Partida(tercLugarDao.getUltimoid(), "não definido", "não definido", "não definido ",carregarSelecao.get(Funcoes.BuscaPosicao(selecaoDao.ListSelecao(),idSelecao1)).getNome(),carregarSelecao.get(Funcoes.BuscaPosicao(selecaoDao.ListSelecao(),idSelecao2)).getNome(),carregarSelecao.get(Funcoes.BuscaPosicao(selecaoDao.ListSelecao(),idSelecao1)).getId(), carregarSelecao.get(Funcoes.BuscaPosicao(selecaoDao.ListSelecao(),idSelecao2)).getId(), 0,0, 0, "não definido",0,0,0,0, 0, false);	
			    tercLugarDao.CreatePart(partida);
			    this.partidas.add(partida);}
    		else{this.LabelError.setText("A disputa pelo 3° lugar já foi criada");}}	
    	else{
    		this.LabelError.setText("É necessário realizar as semifinais para criar a disputa");}}
    	...
    	if(i>=0) {
    		try {
    			...
    			if(this.data.getText().toUpperCase().strip()!="".strip() & this.nomeArbitro.getText().toUpperCase().strip()!="".strip()) {
		    		Partida partida = tercLugarDao.ListPart().get(i);
					...
					tercLugarDao.UpdatePart(NovaPartida, partida.getId());
					...
					if(golsSelecao1== golsSelecao2) {
						...
						try {	
			        		Parent root = FXMLLoader.load(getClass().getResource("/app/view/Penaltis3Lugar.fxml"));
    ...
    void ExcluirAction(ActionEvent event) {
    	...
    	if(i>=0) {
    		...
    		int id = tercLugarDao.ListPart().get(i).getId();
    		tercLugarDao.DeletePart(id);
    ...
    void initialize() {
	    ...
	    partidas.addAll(tercLugarDao.ListPart());
    ...
    public static ArrayList<Partida> getEmpates() {
  	...
  	public static void setEmpates(ArrayList<Partida> empates) {
  		CRUD3LugarController.empates = empates;
  	}      
